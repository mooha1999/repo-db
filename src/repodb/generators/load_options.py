...
def generate_load_options(model: ModelIR, all_models: list[ModelIR]) -> str:
    ...
    for rel in model.relationships:
        if rel.related_model_name in model_names:
            ...
            if rel.back_populates and _is_back_reference(model, rel, all_models):
                fields.append(f"    {rel.attribute_name}: LoadStrategy | bool | None = None")
            else:
                fields.append(
    ...
    lines = [
        '"""Auto-generated load options for {name}. Do not edit manually."""'.format(name=model.class_name),
        "",
        "from __future__ import annotations",
        "",
    ]
...
def _is_back_reference(
    ...
    related = next((m for m in all_models if m.class_name == rel.related_model_name), None)
    if related is None:
        return False
    # If the related model has a relationship pointing back to this model
    for related_rel in related.relationships:
        if related_rel.related_model_name == model.class_name and related_rel.back_populates == rel.attribute_name:
